...
If `COMMENT_IO_LOCAL_SYNC_ROOT` is set, prefer local reads with `rg`, `grep`, `cat`, and `sed` over synced Markdown. Projections are read-only by default. Only browser-approved `read-write-projection` mode permits body-file edits under **My Files**; **Shared With Me, Team Wiki, and Botlets brains always remain read-only**. Ignore any `comment.io:projection` header when constructing API edit text. Use the REST API or web UI for comments, suggestions, revision-checked edits, and every write outside browser-approved My Files write sync.
