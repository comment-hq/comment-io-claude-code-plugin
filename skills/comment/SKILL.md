...
2. **Supplied comm + authenticated HTTPS:** if a clean shortlink hides slug/token, fetch it once with `Accept: text/html, application/json;q=0`, without Authorization or redirects, and accept only an exact token-bearing Comment.io `/d/{slug}` Location. Then extract the token, send `X-Comment-Share-Ingress: 1` on the first personalized `?docs` GET, switch to the returned `your_token`, and read immediately. New share URLs already use a separate handoff token; the header protects legacy anonymous-create links too.
